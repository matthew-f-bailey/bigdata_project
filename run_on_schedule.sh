echo 0. Build out JAR container ===================================
cd on-schedule-example && mvn clean package && cd ..
...
VOLUME=$(pwd)/test_data/on-schedule

echo 2. Build Docker Image for cardexample ===================================
docker build -t hadoop-on-schedule ./on-schedule-example

echo 3. Removing hdfs input dir ===================================
docker run \
    --network docker-hadoop_default \
    --env-file hadoop.env \
    --volume $VOLUME:/local_input \
...
docker run \
    --network docker-hadoop_default \
    --env-file hadoop.env \
    --volume $VOLUME:/local_input bde2020/hadoop-base:master \
    hdfs dfs -cat /output/*
